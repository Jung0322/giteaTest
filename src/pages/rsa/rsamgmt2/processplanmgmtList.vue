<!--
  목적 : 위험성평가 > 공정위험성평가 관리 (Upload) > 평가계획 관리
      ...
      <b-row slot="search">
        ...
        <b-col sm="4" md="4" lg="4" xl="4" class="col-xxl-3">
                ...
                <y-text
                  ...
                  label="평가담당자"
                  name="assessLeaderId"
                  v-model="searchParam.assessLeaderId"
...
export default {
  ...
  methods: {
    ...
    init() {
      ...
      this.excelUrl = selectConfig.rsa.planmgmt.getPlanExcelDown.url;
      // Url Setting
    ...
    closePopupSearchUser(data) {
      ...
      if (data.user) {
        this.searchParam.overUserId = data.user.userId;
        this.searchParam.overUserNm = data.user.userNm;
        this.searchParam.overDeptCd = data.user.deptCd;
        this.searchParam.overDeptNm = data.user.deptNm;
      }
    },
    /**
    ...
    btnDownload() {
      ...
      if (!this.selectedValue || !this.selectedValue.length) {
        window.getApp.$emit('ALERT', {
          ...
          message: 'L0000005853', // 출력할 평가계획을 선택해주세요.
        });
        return;
      }
      this.$_.forEach(this.selectedValue, (item) => {
        if (item.assessStepNm !== '결재완료') {
          window.getApp.$emit('ALERT', {
            title: 'L0000003395', // 안내
            message: '평가계획은 진행단계가 결재완료된 건만 출력 가능합니다.', 
          });
          downFlag = true
          return false;
        }
    ...
    openPopup(data) {
      ...
      this.popupOptions.width = '90%';
      this.popupOptions.visible = true;
      this.popupOptions.title = '평가계획 등록'; // '평가계획 등록';
