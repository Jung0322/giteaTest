  ...
  methods: {
    init() {
      ...
      this.gridOptions.header = [
        ...
        {
          headerText: this.$comm.getLangSpecInfoLabel('L0000002125'), 
          dataField: 'riskBook', 
          width: '10%', 
          renderer: {
            type: 'LinkRenderer',
    ...
    cellClickHandler(event) {
      if (event.dataField !== 'riskBook' && event.dataField !== 'bizApprStepNm' && event.dataField !== 'assessNm') {
        return;
      }
      if (event.dataField === 'riskBook') {
        if (!event.item.riskBook) return;

        this.popupOptions.param = {
          assessPlanNo: event.item.assessPlanNo,
          deptCd: null,
          assessNm: event.item.assessNm,
          assessStatus: event.item.assessStepCd,
          assessTypeNo: event.item.assessTypeNo,
          assessTypeNm: event.item.assessTypeNm,
          bizApprStepCd: event.item.bizApprStepCd,
          processCd: '',
        };

        this.popupOptions.target = () =>
          import(`${'./assessResultDetail.vue'}`);
        this.popupOptions.title = 'L0000002129'; // 위험등록부 등재
        ...
        this.popupOptions.param = {
          ...
          assessNm: event.item.assessNm,
          bizApprStepCd: event.item.bizApprStepCd,
          totalDivision: 'RESULT',
          rapprRqstNo: event.item.rapprRqstNo,
        };
        this.popupOptions.target = () =>
          import(`${'../assessTotal/assessTotalJSADialog.vue'}`);
        this.popupOptions.title = 'L0000003048'; // 평가결과 상세
      }
    ...
    tableLinkAssessNmClicked(header, data) {
      if (header.name === 'riskBook') {
        if (!data.riskBook) return;
