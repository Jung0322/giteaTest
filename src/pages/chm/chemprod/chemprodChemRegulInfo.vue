...
export default {
  ...
  data() {
    return {
      gridOptions: {
        name: 'chemprodChemRegulInfo1',
        name2: 'chemprodChemRegulInfo2',
        header: [],
        data: [],
        height: 300,
        regulItemHeader: [],
        ...
        },
        {
          ...
          labelFunction: (rowIndex, columnIndex, values, headerText, item) => { // HTML 템플릿 작성
            ...
            myThis.regulItems[rowIndex].forEach((row, idx) => {
              ...
              const id = item.regulLawNm + "_A" + row.regulItmNo + "_AA";
              template += " <div class='custom-control custom-control-inline custom-checkbox b-custom-control-sm'> ";
              template += " <input type='checkbox' class='custom-control-input' id='" + id + "' name='" + item.regulLawNm + "_BB' value='" + row.regulItmNo + "'" + checkedTag + disabledTab + ">";
    ...
    setRegulItemData(regulItmNms) {
      ...
      this.$http.request(
        _result => {
          ...
          this.gridOptions.regulItemHeight = 250;
