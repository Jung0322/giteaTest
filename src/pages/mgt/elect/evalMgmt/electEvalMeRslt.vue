          ...
          <y-text
            ...
            :appendIcon="
              changeMeUserEditable
                ...
                : null
            "
            @searchUser="btnSearchUserClicked('meUser')"
            @clearUser="btnClearUser('meUser')"
          ></y-text>
        </b-col>
        <b-col sm="4" md="4" lg="4" xl="4" class="col-xxl-4">
          <!-- 단계/상태 -->
    ...
    <b-row class="grid-height grid-box">
      <b-col sm="12" class="h100p">
        <y-auigrid
          ...
          :height="gridOptions.height"
          :label="this.$comm.getLangSpecInfoLabel('L0000005219')"
...
export default {
  ...
  data: () => ({
    searchParam: {
      ...
      userId: null, // 대상자 사번
      userNm: null, // 대상자
      meUserId: null, // 본인평가자 사번
      meUserNm: null, // 본인평가자
      procStepCd: 'SETS2', // 단계 (본인평가로 고정)
      stateCd: null, // 상태
      evalIncompleteYn: null, // 평가미완료여부
    },
    gridOptions: {
      name: 'electEvalMeRslt',
      btns: [],
      header: [],
      data: [],
      height: 'auto',
  ...
  methods: {
    init() {
      ...
      if (!this.changeMeUserEditable) {
        // 본인평가자
        this.searchParam.meUserId = this.$store.getters.token;
        this.searchParam.meUserNm = this.$store.getters.name;
      }
    ...
    getList() {
      this.$http.url = selectConfig.mgt.electEvalMgmt.rslt.me.list.url;
      this.$http.type = 'GET';
    ...
    cellCLickHandler(event) {
      ...
      } else if (event.dataField === 'meUserNm') {
    ...
    openPopup(data) {
      data.evalType = 'me'; // 본인평가결과

      this.popupOptions.target = () => import(`${'./electEvalRsltDetail.vue'}`);
      this.popupOptions.title = 'L0000005220'; // 본인평가결과 상세
    ...
    closePopupSearchUser(data) {
      ...
      if (data.user) {
        if (this.userType === 'meUser') {
          // 본인평가자 항목일 경우
          this.searchParam.meUserId = data.user.userId;
          this.searchParam.meUserNm = data.user.userNm;
        } else {
          // 대상자 항목일 경우
    ...
    btnClearUser(type) {
      if (type === 'meUser') {
        // 본인평가자 항목일 경우
        this.searchParam.meUserId = '';
        this.searchParam.meUserNm = '';
      } else {
        // 대상자 항목일 경우
