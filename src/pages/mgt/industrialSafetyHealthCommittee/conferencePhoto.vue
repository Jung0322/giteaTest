...
<template>
  <b-container fluid>
    <!-- 등록 -->
    <b-row class="mt-1">
      <b-col v-for="(attachFileGrp, index) in attachFileGrpsPicture" :key="index" :sm="sm" class="mb-2">
        <y-file-upload
...
export default {
      ...
      handler: function(val, oldVal) {
        ...
        if (val !== oldVal) {
          if (this.saveAttachFilesPicture.length > 0) {
            ...
            let _executeIndex = 0;
            let uniqIndex = [];
            this.$_.forEach(this.saveAttachFilesPicture, _item => {
  ...
  methods: {
    ...
    closePopup(data) {
      // set
      // this.$refs[
      //   'fileupload' + this.saveAttachFilesPicture[0].uploadTempFiles.index
      // ][0].getUploadedFileList();
      // 저장이 잘 이루어지게 되면 이곳을 타게 된다.
      //  처음 할 것은 맨처음 데이터를 지운다.
      this.saveAttachFilesPicture.splice(0, 1);
      // 저장할 것이 남았는지 확인한다.
      // if (this.saveAttachFilesPicture.length > 0) {
      //   if (this.saveAttachFilesPicture[0].uploadTempFiles.data.length > 1) {
      //     this.$refs[
      //       'fileupload' + this.saveAttachFilesPicture[0].uploadTempFiles.index
      //     ][0].submitUpload();
      //   }
    ...
    uploadFiles(data, taskClassNm) {
      this.$emit('changeUploadFilesPicture', { data: data, taskClassNm: taskClassNm });
    },
    uploadedList(data, taskClassNm) {
      if (data && data.length > 0) {
        let index = this.$_.findIndex(this.attachFileGrpsPicture, {
