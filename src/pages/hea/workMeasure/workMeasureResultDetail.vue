            ...
            >
              <el-collapse accordion>
                <el-collapse-item name="1">
                  <!-- 업로드결과내역 -->
                  <template slot="title">
                    {{ $comm.getLangSpecInfoLabel('L0000004358') }}&nbsp;&nbsp;
...
export default {
  ...
  data() {
    return {
      ...
      workMeasureChemResults: [],
      workMeasurePhysResults: [],
      popupOptions: {
        target: null,
        title: '',
        visible: false,
  ...
  methods: {
    init() {
      ...
      this.gridOptions.header = [
        {
          text: 'L0000001818', // 실패내용
          name: 'failMessage',
          width: '150px',
        },
        {
          text: 'L0000000688', // 구분코드
          name: 'workAreaGradeCd',
          width: '130px',
          align: 'center',
        },
        {
          text: 'L0000001332', // 부서코드
          name: 'deptCd',
          width: '150px',
          align: 'center',
        },
        {
          text: 'L0000000549', // 공정코드
          name: 'processCd',
          width: '150px',
          align: 'center',
        },
        ...
        {
          text: 'L0000002374', // 작업내용
          name: 'workContents',
          width: '110px',
          align: 'center',
        },
        {
          text: 'L0000000748', // 근로형태(근로시간)
          name: 'shiftType',
          width: '180px',
          align: 'right',
        },
        {
          text: 'L0000001085', // 발생형태 및 발생시간(주기)
          name: 'occurType',
          width: '150px',
          align: 'right',
        },
        {
          text: 'L0000002198', // 유해인자발생시간
          name: 'occurTime',
          width: '150px',
          align: 'right',
        },
        {
          text: 'L0000002948', // 측정위치(근로자명)
          name: 'measPsnNm',
          width: '170px',
          align: 'center',
        },
        {
          text: 'L0000002942', // 측정시간(시작)
          name: 'measStartTime',
          width: '180px',
          align: 'center',
        },
        {
          text: 'L0000002943', // 측정시간(종료)
          name: 'measEndTime',
          width: '180px',
          align: 'center',
        },
        ...
        {
          text: 'L0000000845', // 노출기준
          name: 'exposureStd',
          width: '130px',
          align: 'center',
        },
        {
          text: 'L0000000847', // 노출기준초과여부
          name: 'exposureExcessFlag',
          width: '110px',
          align: 'center',
        },
        {
          text: 'L0000002933', // 측정방법
          name: 'measMethod',
          width: '150px',
        },
        {
    ...
    getDetail() {
      ...
      this.$http.request(
        (_result) => {
          ...
          this.workMeasureChemResults =
            this.workMeasurePlan.workMeasureResults.filter((item) => {
              return item.workAreaGradeCd === 'C';
            });
          this.workMeasurePhysResults =
            this.workMeasurePlan.workMeasureResults.filter((item) => {
              return item.workAreaGradeCd === 'P';
            });
        },
        (_error) => {
    ...
    submitUpload() {
      ...
      window.getApp.$emit('CONFIRM', {
        ...
        message: 'M0000001041', // 엑셀 업로드시 기존에 등록한 내역은 초기화 됩니다. 업로드 하시겠습니까?
        type: 'warning',
        confirmCallback: () => {
          this.loading = true;
    ...
    getConfirmMsg(response) {
      ...
      if (response.totCount === 0) {
        ...
        returnMsg = String(
          this.$comm.getLangSpecInfoLabel('L0000002860') + // 총
            response.totCount +
            this.$comm.getLangSpecInfoLabel('L0000004359') + // 건 (완료 :
            response.completeCount +
            this.$comm.getLangSpecInfoLabel('L0000004360') + // , 실패 :
            response.dontCount +
            this.$comm.getLangSpecInfoLabel('L0000004361') // )이 업로드 되었습니다.
        );
      }
      return returnMsg;
    },
    apprBtnClicked() {
      let promises = [
        // { func: this.checkResult },
        { func: this.confirmAppr },
        { func: this.openAppr },
      ];
      this.$comm.orderedPromise(promises);
    },
    // 결재요청 전 결과값 입력 검사 (필요시 활성화하여 로직 추가할 것)
    // checkResult() {
    ...
    confirmAppr() {
      return new Promise((_resolve, _reject) => {
        window.getApp.$emit('CONFIRM', {
          title: 'L0000003321', // '확인',
          message: 'M0000000088', // 결재 요청하시겠습니까?
          type: 'info',
          // 확인 callback 함수
          confirmCallback: () => {
            _resolve(true);
    ...
    saveData() {
      return new Promise((resolve) => {
        this.$_.forEach(
          this.workMeasurePlan.workMeasureResults,
          (item) => (item.updateUserId = this.$store.getters.token)
        );
