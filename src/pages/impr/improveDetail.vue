        ...
        <b-row>
          <b-col sm="12">
            ...
            <div slot="buttonGroup" class="float-right mb-1">
              <!--<el-tag
                v-if="popupParam.bizApprStepCd === 'BAPSD'"
                size="mini"
                type="danger"
              >
                <i class="el-icon-warning"></i>
                {{ $comm.getLangSpecInfoLabel('L0000003570') }}
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      this.$http.request(
        (_result) => {
          ...
          if (
            this.imprAct.imprStepCd === 'IMST5' ||
            this.popupParam.flag === 'DETAIL' ||
            this.imprAct.bizApprStepCd === 'BAPSA' ||
            this.imprAct.actUserId.toLowerCase() !==
              this.$store.getters.token.toLowerCase()
          ) {
            this.disabled = true;
          } else {
            this.disabled = false;
    ...
    closePopup(data) {
      if (data && data.beforeAfterComplete) {
        this.saveAttach();
      } else {
    ...
    btnAppr() {
      let returnVal = this.tabDataCheck();
      if (returnVal) {
        return;
      } else {
        // 조치협력업체를 지정하고 협력업체는 아직 완료처리를 하지 않은 경우 결재를 할 수 없다.
        if (
          this.imprAct.actVendorConfirmYn !== 'Y' &&
          this.imprAct.actVendorCd
        ) {
          window.getApp.$emit('ALERT', {
            title: 'L0000003395' /* 안내 */,
            message:
              'M0000000485' /* 조치협력업체가 아직 완료처리를 하지 않았습니다. */,
            type: 'warning', // success / info / warning / error
          });
        } else {
          this.$validator
              ...
              if (_result) {
                window.getApp.$emit('CONFIRM', {
                  ...
                  confirmCallback: () => {
                    ...
                    this.$http.url = transactionConfig.saf.imprAct.edit.url;
                    this.$http.type = 'PUT';
                    ...
                    this.$http.request(
                      (_result) => {
                        ...
                        this.popupOptions.param = {
                          apprBizCd: 'CR_IM-02',
                          apprRqstNo: this.imprAct.apprRqstNo,
                          apprParams: {
                            safImprActNo: this.popupParam.safImprActNo,
                          },
                        };
                      },
                      (_error) => {
