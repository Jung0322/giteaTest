...
export default {
  ...
  methods: {
    init() {
      ...
      // URL 셋팅
      this.searchUrl = selectConfig.saf.education.resultEdu.list.url;

      // 권한에 따른 대상,수행 부서 검색 초기화
            ...
            this.gridOptions.data = this.$_.clone(
              _result.data.filter(
                (v) =>
                  ...
                  v.execDeptCd === this.$store.getters.deptCd ||
                  v.rstepCd === 'BAPSG'
              )
            );
            this.YAuiGrid.setGridData(this.gridOptions.data);
    ...
    cellClickHandler(event) {
      if (event.dataField === 'qrCode') {
      ...
      } else if (event.dataField === 'rstateNm') {
        // 결재 상태 팝업
        this.showApprProgressPopup(event.item);
      } else if (event.dataField === 'eduNm') {
        this.popupOptions.target = () => import(`${'./eduResult.vue'}`);
        ...
        this.popupOptions.title = 'L0000002216'; /* 이수인원(총합) */
        this.popupOptions.visible = true;
        this.popupOptions.width = '80%';
        this.popupOptions.top = '10px';
        this.popupOptions.param = event.item;
    ...
    tableLinkClicked(header, data) {
      ...
      if (header.name === 'qrCode') {
        ...
        this.popupOptions.closeCallback = this.closePopup;
      } else if (header.name === 'rstateNm') {
        // 결재 상태 팝업
        this.showApprProgressPopup(data);
      } else if (header.name === 'eduNm') {
        this.popupOptions.target = () => import(`${'./eduResult.vue'}`);
        this.popupOptions.title = 'L0000000626'; /* 교육결과 등록/수정 */
        this.popupOptions.visible = true;
        this.popupOptions.width = '80%';
        ...
        header.name === 'totalUserNum' ||
        header.name === 'passUserNum' ||
        header.name === 'failUserNum' ||
        header.name === 'exceptUserNum'
      ) {
        this.popupOptions.target = () => import(`${'./eduUserListPopup.vue'}`);
        if (header.name === 'totalUserNum') {
          this.popupOptions.title = 'L0000000934'; // 대상인원
        } else if (header.name === 'passUserNum') {
          this.popupOptions.title = 'L0000002213'; // 이수인원
          data.completCheckYn = 'Y';
        } else if (header.name === 'failUserNum') {
          this.popupOptions.title = 'L0000001354'; // 불참인원
          data.completCheckYn = 'N';
        } else if (header.name === 'exceptUserNum') {
          this.popupOptions.title = 'L0000002622'; // 제외인원
          data.notReEduYn = 'Y';
          data.completCheckYn = 'N';
        }
        ...
        this.popupOptions.param = data;
        this.popupOptions.closeCallback = this.closePopup;
      } else if (header.name === 'passOutUserNum') {
        // 이수인원(외부) 팝업
        this.popupOptions.target = () =>
          import(`${'./eduOutUserListPopup.vue'}`);
        this.popupOptions.title = 'L0000002215'; /* 이수인원(외부) */
        ...
        this.popupOptions.param = data;
        this.popupOptions.closeCallback = this.closePopup;
      } else if (header.name === 'totPassUserNum') {
        // 이수인원(총합) 팝업
        this.popupOptions.target = () => import(`${'./eduComYListPopup.vue'}`);
        this.popupOptions.title = 'L0000002216'; /* 이수인원(총합) */
        this.popupOptions.visible = true;
