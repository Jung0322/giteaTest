      ...
      <b-row slot="search">
        ...
        <b-col sm="6" md="6" lg="6" xl="6" class="col-xxl-3">
          <y-text
            ...
            label="L0000002000"
            name="eairFuelNm"
            v-model="searchParam.eairFuelNm"
          ></y-text>
        </b-col>
      </b-row>
    </y-search-box>
...
export default {
  ...
  methods: {
    init() {
      ...
      this.gridOptions.header = [
        ...
        // 발열량단위
        {
          ...
          dataField: 'calValUnitNm',
          width: '10%',
          style: 'center-align',
        },
        // 황함량(%)
        {
          headerText: this.$comm.getLangSpecInfoLabel('L0000003358'),
          dataField: 'sulfurContent',
          width: '10%',
          style: 'center-align',
        },
        // SEMS 코드
        {
          headerText: this.$comm.getLangSpecInfoLabel('L0000000161'),
          dataField: 'semsCd',
          width: '13%',
          style: 'center-align',
        },
    ...
    tableLinkClicked(data) {
      if (!data) return;
      if (data.dataField === 'eairFuelNm') {
        this.btnPopupClickedCallback(data.item);
      }
    },
    btnPopupClickedCallback(data) {
      this.popupOptions.target = () => import(`${'./fuelDetail.vue'}`);
      this.popupOptions.title = 'L0000001998'; // '연료 상세 등록/수정';
      this.popupOptions.visible = true;
      this.popupOptions.param = {
        eairFuelCd: data ? data.eairFuelCd : null,
      };
