        ...
        <b-row>
          <b-col sm="12">
            ...
            <div slot="buttonGroup" class="float-right mb-1">
              ...
              <!-- 일지출력 -->
              <y-btn
                v-if="editable && updateMode && buttonAble"
                title="L0000002283"
...
export default {
  ...
  methods: {
    init() {
      ...
      if (this.popupParam.measureYmd && this.popupParam.deptCd) {
    ...
    getDetail() {
      this.$http.url = this.$format(
        selectConfig.env.air.opLog.oplogAll.url,
      ...
      this.$http.request(
        (_result) => {
          ...
          if (this.opLogRslt.bizApprStepCd === 'BAPSG') {
            this.buttonAble = true;
          } else {
            this.buttonAble = false;
          }
          if (this.opLogRslt.createDt) {
      ...
      this.$validator
        ...
        .then((_result) => {
          if (_result) {
            ...
            this.$http.param = {
              plantCd: this.opLogRslt.plantCd,
              measureYmd: this.opLogRslt.measureYmd,
              deptCd: this.opLogRslt.deptCd,
            };
            this.$http.type = 'GET';
            this.$http.request(
              (_result) => {
    ...
    btnApprClicked() {
      window.getApp.$emit('CONFIRM', {
        ...
        confirmCallback: () => {
          this.$http.url =
            transactionConfig.env.air.opLog.createOplogResult.edit.url;
          this.$http.type = 'PUT';
