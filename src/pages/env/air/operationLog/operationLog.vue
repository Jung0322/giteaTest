...
export default {
  ...
  methods: {
    init() {
      ...
      this.searchUrl = selectConfig.env.air.opLog.list.url;
    ...
    getList() {
      this.searchParam.fromDate = this.searchParam.evalPeriod[0];
      this.searchParam.toDate = this.searchParam.evalPeriod[1];
    ...
    openPopup(header, data) {
      if (header === 'envOpLogStNm') {
        ...
        this.popupOptions.closeCallback = this.closePopup;
      } else if (header === 'editDisch') {
        this.popupOptions.target = () =>
          import(`${'./dischargeChkResultDetail.vue'}`);
        this.popupOptions.title = 'L0000004222'; // 배출구별 가동시간
        this.popupOptions.width = '80%';
        this.popupOptions.top = '10px';

        this.popupOptions.param = {
          selectedMeasureYmd: data.measureYmd,
          selectedDeptCd: data.deptCd,
          selectedPlantCd: data.plantCd,
        };
        this.popupOptions.visible = true;

        this.popupOptions.closeCallback = this.closePopup;
      } else if (header === 'editPrevent') {
        this.popupOptions.target = () =>
          import(`${'./preventFacOperChkResultDetail.vue'}`);
        this.popupOptions.title = 'L0000004223'; // 방지시설 가동시간
        this.popupOptions.width = '80%';
        this.popupOptions.top = '10px';
        ...
        this.popupOptions.closeCallback = this.closePopup;
      } else if (header === 'editFuelIngr') {
        this.popupOptions.target = () =>
          import(`${'./fuelIngrChkResultDetail.vue'}`);
        this.popupOptions.title = 'L0000004224'; // 연료 원료 사용량
        this.popupOptions.width = '80%';
        this.popupOptions.top = '10px';
