<!--
  목적 : 안전 > 기준정보 > 발생형태(대분류/중분류)
  작성자 : kdh
  Detail : 발생형태(대분류/중분류) 관리
  *
  examples:
  *
  -->
    ...
    <b-row>
      <b-col sm="12">
        <y-auigrid
          ...
          :label="this.$comm.getLangSpecInfoLabel('L0000001087')"
          :useExcelExport="true"
...
export default {
  ...
  data() {
    return {
      ...
      YAuiGrid: null,
      gridOptions: {
        name: 'safBaseInfoAccidentOccurrForm',
        header: [],
        data: [],
      },
      useItems: [],
  ...
  methods: {
    init() {
      ...
      this.searchUrl = selectConfig.saf.accidentOccurrForm.list.url;
    ...
    cellClickHandler(event) {
      ...
      this.popupOptions.target = () => import(`${'./occurrFormDetail.vue'}`);
      this.popupOptions.title = 'L0000001083'; // 발생형태 상세 등록/수정
