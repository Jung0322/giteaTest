...
export default {
  ...
  data: () => ({
    ...
    YAuiGrid: null,
    gridOptions: {
      name: 'safConsolidationConsolidationPlans',
      header: [],
      data: [],
      height: '300',
    },
  ...
  methods: {
    ...
    cellClickHandler(event) {
      if (event.dataField === 'chkTitle') {
        this.popupOptions.target = () =>
          import(`${'./consolidationPlanDetail.vue'}`);
        this.popupOptions.title = '합동점검계획 수정/상세';
        this.popupOptions.param = {
          safCongChkRsltNo: event.item.safCongChkRsltNo, // 합동순회점검일련번호
        };
    ...
    openPopUp() {
      this.popupOptions.target = () =>
        import(`${'./consolidationPlanDetail.vue'}`);
      this.popupOptions.title = '합동점검계획 등록';
      this.popupOptions.param = {
        safCongChkRsltNo: 0, // 합동순회점검일련번호
      };
      this.popupOptions.visible = true;
      this.popupOptions.closeCallback = this.closePopup;
    },
