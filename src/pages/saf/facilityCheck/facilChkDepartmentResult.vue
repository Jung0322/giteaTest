  ...
  methods: {
    init() {
      ...
      this.gridOptions.merge.push(
        // { index: 0, field: 'rownum' },px',
        { index: 0, field: 'pmergeKey' },
        { index: 1, field: 'pmergeKey' },
        { index: 2, field: 'pmergeKey' },
        { index: 3, field: 'pmergeKey' },
        { index: 4, field: 'pmergeKey' },
        { index: 5, field: 'pmergeKey' },
        { index: 9, field: 'pmergeKey' },
        { index: 10, field: 'pmergeKey' }
      );
    ...
    tableLinkCheckTitleClicked(header, data) {
      ...
      this.popupOptions.param = {
        safFacilChkNo: data ? data.safFacilChkNo : 0, // 설비점검번호
        noPlanYn: data ? 'N' : 'Y', // 무계획결과등록여부
        pDeptCd: data ? data.pdeptCd : '', // 점검수행부서코드
        pPlantCd: data ? data.pplantCd : '', // 점검수행사업장코드
    ...
    openDetailPopup(event) {
      ...
      this.popupOptions.param = {
        ...
        pDeptCd: event.item ? event.item.pdeptCd : '', // 점검수행부서코드
        pPlantCd: event.item ? event.item.pplantCd : '', // 점검수행사업장코드
        planFlag: true, // 실적에서 계획을 눌렀을떄 수정못하게 하기 위한 Flag
      };
