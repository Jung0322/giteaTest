...
export default {
  ...
  methods: {
    tableRowClassName({ row, rowIndex }) {
      ...
      if (row.gubun === '점검 실시율(%)' || row.gubun === '개선 조치율(%)') {
    ...
    init() {
      ...
      this.searchUrl = selectConfig.saf.inspectionresult.getstatus.url;
    ...
    getHeaders() {
      ...
      this.gridOptions.header.push({
        ...
        styleFunction: (rowIndex, columnIndex, value, headerText, item, dataField) => {
          if (item.gubun === '점검 실시율(%)') {
            return "row-color3";
          } else if (item.gubun === '개선 조치율(%)') {
            return "row-color3";
          } else if (item.plant_nm === '합계') {
            return "row-color3";
          } else if (item.gubun === '기한초과(건수)') {
            if (item.oldCnt > 0 && item.oldCnt !== '0') {
              return "row-color";
            } else {
              return "row-color2";
            }            
      ...
      this.gridOptions.header.push({
        ...
        styleFunction: (rowIndex, columnIndex, value, headerText, item, dataField) => {
          if (item.gubun === '점검 실시율(%)') {
            ...
            if (item.allCnt > 0 && item.allCnt !== '0') {
              return "row-color";
            } else {
              return "row-color2";
            }            
    ...
    cellClickHandler(event) {
      ...
      if (event.item.gubun === '점검 실시율(%)') return;
      ...
      if (event.item.rsa_result === '개선현황') {
        ...
        event.item.imprClassCd = 'ICL38';
      }
      // data.actLimitYmd = this.searchParam.year;
      ...
      let strParam = event.item.subPlantCd + '|' + event.item.regRegdemCd;
    ...
    tableLinkClicked(header, data) {
      ...
      let strParam = data.subPlantCd + '|' + data.regRegdemCd;
    ...
    openResultPopup(data) {
      ...
      this.popupOptions.width = '90%';
      this.popupOptions.visible = true;
      this.popupOptions.title = 'L0000005250'; // '점검계획/결과 관리';
