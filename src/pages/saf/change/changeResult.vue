...
export default {
  ...
  methods: {
    init() {
      ...
      // 진행단계
      this.$comm.getComboItems('SAF_CHNG_STEP').then((_result) => {
        _result.splice(this.$_.findIndex(_result, { code: 'CGSP1' }), 1);
        _result.splice(this.$_.findIndex(_result, { code: 'CGSP2' }), 1);
        _result.splice(0, 0, {
          code: 'CGSP99',
          codeNm: this.$comm.getLangSpecInfoLabel('L0000002519'),
        });
        this.chngStepCdItems = _result;
      });
      // 변경구분
    ...
    cellClickHandler(event) {
      if (event.dataField === 'bizApprStepNm') {
        ...
        this.popupOptions.param = {
          safChngNo: event.item.safChngNo, // key
          chngStepCd: event.item.chngStepCd, // 진행상태
        };
