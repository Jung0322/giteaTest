...
export default {
  ...
  name: 'self-measurement-detail-appr',
  components: {},
  props: {
    popupParam: {
      type: Object,
  ...
  methods: {
    init() {
      ...
      this.getEairOutletNoItems();
      // 자가측정방법
      this.$comm.getComboItems('EAIR_SELF_TEST_MTD', false).then((_result) => {
      ...
      this.$comm.getComboItems('EAIR_INST', false).then((_result) => {
        ...
        this.$comm.getComboItems('EAIR_TEST_MTD', false).then((_result) => {
          ...
          this.gridOptions.header = [
            ...
            // 단위
            { text: 'L0000000870', name: 'envUnitNm', width: '100px' },
            // 배출량계산팩터
            {
              text: 'L0000001157',
      ...
      if (this.popupParam.eairOpMeasNo) {
              ...
              this.$http.request(
                (_result) => {
                  if (_result.data > 0) {
                    ...
                    this.selfMeasure.updateUserId = this.$store.getters.token;
                    this.selfMeasure.selfMeasurementResult =
                      this.gridOptions.data;

                    this.$http.url = this.saveUrl;
                    this.$http.param = this.selfMeasure;
                    this.$http.type = 'PUT';
                    this.$http.request(
                      (_result) => {
    ...
    getList() {
      ...
      this.$http.url = this.$format(
        selectConfig.env.air.baseInfo.outlet.get.url,
      ...
      this.$http.request(
        (_result) => {
          ...
          if (
            ...
            this.selfMeasure.bizApprStepCd === 'BAPSG'
          ) {
            this.disabled = true;
          }
        },
