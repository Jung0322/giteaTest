<!--
  ...
  Detail : 안전점검일정 등록/수정 화면
           ...
           **
           2021-05-14 jkl
           현재 화면을 수정할 경우 업무용 vue파일도 같이 현행화 해야함
           checkScheduleDetail.vue
           **
...
export default {
  name: 'check-schedule-detail-appr',
  props: {
    popupParam: {
      type: Object,
      default: {
  ...
  methods: {
    init() {
      ...
      this.tgtDeptGridOptions.header = [
        // 부서
        { text: 'L0000001287', name: 'deptNm', width: '100%', align: 'center' },
      ];
      ...
      this.$http.request(
        (_result) => {
          ...
          if (this.checkResult.checkScheduleList.length > 0) {
            ...
            console.log(this.checkResult.checkScheduleList);
    ...
    btnDeleteClickedCallback(_result) {
      window.getApp.$emit('CONFIRM', {
        title: 'L0000003321', // 확인
        message: 'M0000000716', // 안전점검 일정과 생성된 안전점검계획들이 삭제됩니다.\n삭제하시겠습니까?
        type: 'info', // success / info / warning / error
        confirmCallback: () => {
          this.$http.url = this.$format(
    ...
    btnDeptDelete() {
      ...
      if (this.selectedValue.length === 0) {
        ...
        this.confirm({
          title: 'L0000003321', // 확인
          message: 'M0000000531', // 하위부서와 같이 제외하시겠습니까?
          type: 'info',
          // 확인 callback 함수 (선택된 부서의 하위부서도 같이 삭제)
          confirmCallback: () => {
            this.$_.forEach(this.tgtDeptGridOptions.data, (item) => {
              let idx = '';
              let sIdx = '';
              ...
              sIdx = this.$_.findIndex(this.selectedValue, {
                deptCd: item.deptCd,
              });

              if (idx > -1 && sIdx < 0) {
                targetData.splice(0, 0, item);
                this.selectedValue.splice(0, 0, item);
              }
            });
            this.$refs.dataTable.toggleSelection(targetData);

            this.$_.forEach(this.selectedValue, (item) => {
              this.tgtDeptGridOptions.data = this.$_.clone(
                this.$_.reject(this.tgtDeptGridOptions.data, item)
              );
            });
          },
