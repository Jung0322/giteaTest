...
export default {
  ...
  name: 'y-vue-simple-calendar', // 반드시 입력하세요(안 하면 warning 발생). 네이밍 룰은 현재 vue component의 파일명의 단어를 "-"로 구분(예:exam-data)하여 입력하시면 됩니다. 입력후 이 주석은 지워주세요.
  props: {
    calItems: {
      type: Array,
      default: () => [],
  ...
  methods: {
    ...
    init() {
      ...
      this.vShowDate = this.thisMonth(1);
      this.makeCalendar();
    },
    /** /초기화 관련 함수 **/
    periodChanged() {
    ...
    onSearch() {
      this.$nextTick(() => {
        ...
        // }
        this.$emit('onSearch', this.vShowDate);
      });
    },
  },
};
...
.theme-default .previousYear,
.theme-default .currentPeriod,
.theme-default .nextPeriod,
.theme-default .nextYear,
.theme-default .previousPeriod {
...
/*MGT_CAL_KIND:교육 */
.theme-default .cv-event.rb {
  background-color: #9ea623;
  border-color: #9ea623;
  color: #fff;
}
/*MGT_CAL_KIND:점검 */
.theme-default .cv-event.gr {
  background-color: #ff8840;
  border-color: #ff8840;
  color: #fff;
}
/*MGT_CAL_KIND:개선 */
.theme-default .cv-event.bl {
  background-color: #69b341;
  border-color: #69b341;
  color: #fff;
}
/*MGT_CAL_KIND:변경 */
.theme-default .cv-event.ye {
  background-color: #ff5652;
  border-color: #ff5652;
  color: #fff;
}
/*MGT_CAL_KIND:보건 */
.theme-default .cv-event.pu {
  background-color: #59adf2;
  border-color: #59adf2;
  color: #fff;
}
/*MGT_CAL_KIND:평가 */
.theme-default .cv-event.or {
  background-color: #879be8;
  border-color: #879be8;
  color: #fff;
}
/*MGT_CAL_KIND:기타 */
.theme-default .cv-event.kh {
